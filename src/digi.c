...
*/
static DigiType _usedDigiType;
static uint16_t _useDigiRate;
...
//				out= 0;	redundant
		...
		if (assertSameSource(voice+1))recordSample(out);
...
uint8_t digiIsMahoneyMode() {
	return (_usedDigiType == DigiMahoneyD418);
}
...
static uint8_t getD418Sample( uint8_t value) {
	...
	if (cpuGetProgramMode() == NMI_OFFSET_MASK) {
		...
		*/
		if (addr == 0xd418) {	// traditional digis
			...
			if (assertSameSource(0)) recordSample(isMahoneyDigi() ? _mahoneySample[value] : getD418Sample(value));
		...
		if (!envIsPSID() && (addr == 0xd418)) {
			if(assertSameSource(0)) recordSample(isMahoneyDigi() ? _mahoneySample[value] : getD418Sample(value));	// this may lead to false positives..
		}					
		if (!_isC64compatible) {
...
uint8_t digiRenderSamples(uint8_t * digiBuffer, uint32_t cyclesPerScreen, uint16_t samplesPerCall) {	
	...
	*/		
	if (_digiCount > SAMPLE_DETECT_MIN) {
		...
		// some diagnostic info for use in GUI
		_useDigiRate= round(envFPS() *_digiCount);
...
*/
static int16_t genDigi(int16_t in, uint8_t digi, uint8_t is6581) {
