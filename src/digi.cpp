...
uint8_t DigiDetector::handleIceGuysDigi(uint8_t voice, uint8_t reg, uint8_t value) {
	/* 
	...
	problem: song depends on reading the "d41b" OSC3 and the "correct" impl (e.g. what pollytracker
	expects) makes it go mute (maybe an issue of Hermit's WF antialiasing or the combined
	tri/saw/pulse WF impl).
	*/
	if ((reg == 4 ) && !(_sid->getWave(voice)&0x8) && (_sid->getAD(voice) == 0x0f)){	
		
		uint8_t out= 0;
		switch(value) {	// no idea about the correct output levels generated by this approach..
			case 0x71:		// idx 0	(WF: tri/saw/pulse)
//				out= 0;	redundant
				break;
			case 0x21:		// idx 1	(WF: saw)
				out= 0x44;
				break;
			case 0x11:		// idx 2	(WF: tri)
				out= 0x88;
				break;
			case 0x41:		// idx 3	(WF: pulse)
				out= 0xcc;
				break;
			default:
				return 0;	// false positive, e.g. 	Wonderland_XII-Digi_part_1.sid	
		}
		_usedDigiType = DigiWFM;

		if (assertSameSource(voice+1)) recordSample(out, voice+1);
...
uint8_t DigiDetector::detectSample(uint16_t addr, uint8_t value) {
	...
	// already excluded below.. isn't it?
// XXX	if (_isC64compatible && !((addr>=0xd400) && (addr<= 0xd41c))) return 0;	// only use regular SID for digi (exclude PSID shit which uses weird addresses)
